...
def coth(x):
    if np.cosh(x) == np.inf and np.sinh(x) == np.inf:
        return 1
    else:
...
class DCCircuit:
    ...
    def _print_params(self):
        print(f'Ejp:    {self.Ejp * 1e-9 / self.h} GHz')
        print(f'Ejt:    {self.Ejt * 1e-9 / self.h} GHz')
    ...
    def calc_omega01(self, update=False):
        ...
        return np.real(self.evals_p[1] - self.evals_p[0]) / self.h

    def calc_capacitive_loss_rate(self, capacitance, omega01, T=4e-6):
        Q_cap = 1e6 * (6e9 / np.abs(omega01))**0.7
        S = (self.hbar / (capacitance * Q_cap)) * (1 + coth((self.h * omega01) / (2 * self.kb * T)))

        noise_op = self.tensor3(self.n_op_cb, self.n_op_cb, self.n_op_cb)
        noise_coupling = np.abs(self.hc(self.probe_1_eb).dot(noise_op.dot(self.probe_0_eb)).toarray()[0][0])**2

        print(S)
        print(noise_coupling)

        rate = (1 / self.hbar)**2 * noise_coupling * S

        return rate

    def calc_T1(self, update=True):
        # self.ng = ng0
        # self.flux = flux0
        self.init_qubit_states(update=True)
